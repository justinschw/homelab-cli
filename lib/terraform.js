...
Terraform.prototype.populateVmIds = function (strConfig, destroy = false) {
  ...
  for (let i = 0; i < matchesArray.length; i++) {
    ...
    if (existing) {
      strConfig = strConfig.replace(originalMatch, existing.vmid);
...
Terraform.prototype.populateIpAddresses = function (strConfig, destroy = false) {
  ...
  for (let i = 0; i < matchesArray.length; i++) {
    ...
    if (existing) {
      strConfig = strConfig.replace(originalMatch, existing.ip);
...
Terraform.prototype.apply = async function (log = true) {
  ...
  // Add to inventory
  updateInventoryWithNewEntries(this.config.tf.inventoryFile, this.newInventory, this.deletedInventory);
};
