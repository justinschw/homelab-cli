...
function Terraform(config) {
  const schema = joi.object({
    ...
    tf: joi.object({
      ...
      stateFileName: joi.string().default('terraform.tfstate'),
...
Terraform.prototype.populateVmIds = function (strConfig, destroy = false) {
  ...
  for (let i = 0; i < matchesArray.length; i++) {
    ...
    let vmid;
    if (destroy) {
      vmid = releaseVmId(this.inventory, element);
      console.info(`Released VM ID ${vmid} from inventory for destruction`);
    } else {
      vmid = reserveVmId(type, this.inventory, element);
      console.info(`Reserved VM ID ${vmid} in inventory`);
    }
    strConfig = strConfig.replace(originalMatch, vmid);
  }
...
Terraform.prototype.populateIpAddresses = function (strConfig, destroy = false) {
  ...
  for (let i = 0; i < matchesArray.length; i++) {
    ...
    let ip;
    if (destroy) {
      ip = releaseIpAddress(this.inventory, element);
      console.info(`Released IP address ${ip} from inventory for destruction`);
    } else {
      ip = reserveIpAddress(element.split(':')[1], this.inventory, element);
      console.info(`Reserved IP address ${ip} in inventory`);
    }
    strConfig = strConfig.replace(originalMatch, ip);
  }
...
Terraform.prototype.init = async function (destroy = false) {
  ...
  this.inventory = loadInventory(this.config.tf.inventoryFile);
  if (!this.inventory) {
  ...
  console.info(`Cloning Terraform module ${this.config.tf.module} into ${tmpdir}...`);
  const cloneArgs = ['clone', this.config.tf.module, '-b', this.config.tf.branch, tmpdir];
  await this.command.run('git', cloneArgs, { log: false });
  this.clonedDir = tmpdir;
  this.moduleDir = path.resolve(tmpdir, this.config.tf.path || '.');
};
...
Terraform.prototype.setBackendVars = function (args) {
  ...
  // set key
  args.push(`-backend-config=key=${this.config.tf.stateFileName}`);
}

Terraform.prototype.setVariables = function (env) {
  for (const [key, value] of Object.entries(this.config.tf.variables)) {
...
Terraform.prototype.apply = async function (log = true) {
  ...
  // Add to inventory
  updateInventory(this.config.tf.inventoryFile, this.inventory);
};
