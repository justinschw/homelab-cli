...
Bw.prototype.fill = async function(config) {
    ...
    for (let i = 0; i < bwMatches.length; i++) {
        let element = bwMatches[i].replace(/{|}/g, '');
        // see if base64 encode is requested
        const isBase64 = element.endsWith(':base64');
        if (isBase64) {
            // remove :base64 suffix for lookup
        ...
        if (typeof current === 'string') {
            ...
            strConfig = strConfig.replace(bwMatches[i], `${current}`);
        }
    }
