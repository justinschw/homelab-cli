...
function populateUpperLevelRefs(config) {
  ...
  for (let i = 0; i < upperMatches.length; i++) {
    ...
    const result = getRefValue(element.slice('../'.length), config);
    strConfig = strConfig.replace(upperMatches[i], result);
  }
  return JSON.parse(strConfig);
}

function updateInventoryWithNewEntries(inventoryFile, newEntries, deletedEntries) {
  const schema = joi.object({
    vmids: joi.array().items(joi.object({
      vmid: joi.number().integer().required(),
      refId: joi.string().required()
  ...
  const { error: addError, value: added } = schema.validate(newEntries);
  if (addError) {
    throw new Error(`Invalid new entries for inventory update: ${addError.message}`);
  }
  const { error: delError, value: deleted } = schema.validate(deletedEntries || { vmids: [], ips: [] });
  if (delError) {
    throw new Error(`Invalid deleted entries for inventory update: ${delError.message}`);
  }
  let inventory = loadInventory(inventoryFile);
  if (!inventory) {
    throw new Error('Failed to load inventory for update');
  }
  if (!inventory.reserved) inventory.reserved = {
    vmids: [],
    ips: []
  };
  added.vmids.forEach(v => {
    const existing = inventory.reserved.vmids.find(item => item.refId === v.refId);
    if (!existing) {
      inventory.reserved.vmids.push(v);
    } else {
      Object.assign(existing, v);
    }
  });

  added.ips.forEach(ip => {
    const existing = inventory.reserved.ips.find(item => item.refId === ip.refId);
    if (!existing) {
      inventory.reserved.ips.push(ip);
    } else {
      Object.assign(existing, ip);
    }
  });

  deleted.vmids.forEach(v => {
    const index = inventory.reserved.vmids.findIndex(item => item.refId === v.refId);
    if (index !== -1) {
      inventory.reserved.vmids.splice(index, 1);
    }
  });

  deleted.ips.forEach(ip => {
    const index = inventory.reserved.ips.findIndex(item => item.refId === ip.refId);
    if (index !== -1) {
