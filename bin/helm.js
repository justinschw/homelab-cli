...
function usage() {
    console.error(`Usage: helm.js --manifest <manifest.json> [--bw-auth <auth.json>]`);
    console.error(`
Flags:
...
(async function main() {
    ...
    try {
        ...
        if (args.inventoryFile) {
            ...
            manifest.helm.homelab_inventory_file = inventoryPath;
        }
        const Helm = require('../lib/helm');
        helm = new Helm(manifest);
        await helm.init();
        await helm.helmInstall();
        await helm.cleanup();
        process.exit(0);
    } catch (err) {
        console.error('Helm deploy failed:', err && err.message ? err.message : err);
        if (helm) await helm.cleanup();
        process.exit(1);
    }
})();
